...
//    Explicitly set SignatureAlgorithm (ECDSAWithSHA256 or SHA256WithRSA) per CON-INV-008.
...
// 6. Explicitly set SignatureAlgorithm on the RevocationList template (CON-INV-008).
// 7. x509.CreateRevocationList (signed with CA key, SHA-256 — CON-INV-005, CON-INV-008).
// STAGE SUB-PHASE (ADR-006):
// 8. Stage CRL PEM → ca.crl.tmp.
// 9. Stage crlnumber counter (crlnumber + 1) → crlnumber.tmp.
//    If any stage write fails: cleanupTempFiles, return error.
// COMMIT SUB-PHASE (ADR-006):
// 10. Rename ca.crl.tmp → ca.crl.       (CRL updated first)
// 11. Rename crlnumber.tmp → crlnumber. (counter advanced)
// 12. Return CRLResult.
```
...
| [ADR-006](ADRs/ADR-006-atomic-replace-for-mutate-phase-writes.md) | Atomic Replace for Mutate-Phase Writes | Use temp-file-then-rename for all persistent writes; stage-then-commit protocol for multi-file mutations; reject WAL and in-memory flush |
...
| CON-INV-008 | CA Operations + CRL | ca.go, crl.go | Certificate and CRL templates explicitly set `SignatureAlgorithm` to `x509.ECDSAWithSHA256` (ECDSA) or `x509.SHA256WithRSA` (RSA) based on the CA key type; never relies on auto-detection |
...
| CON-DI-010 | CA Operations | ca.go | Certificate templates explicitly set `Version: 3` (the `x509.Certificate.Version` field uses 0-indexed values, so the literal is `2` for X.509v3, but Go's `CreateCertificate` forces v3 when extensions are present; the explicit assignment documents intent) |
