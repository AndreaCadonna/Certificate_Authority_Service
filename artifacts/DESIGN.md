...
// Writes data to a temp file in the same directory as path (same filesystem),
// then atomically renames it over path using os.Rename.
// If any step fails: removes temp file, returns error.
// Guarantees: target file has either old content or new content, never partial.

func RemoveIfExists(path string) error
// Removes the file at path if it exists. Returns nil if file does not exist.
// Used in cleanup paths for ordered multi-file mutations (ADR-003 Layer 2).
```
...
// MUTATE PHASE (atomic writes + ordered cleanup per ADR-003):
...
// 10. SaveCertPEM to certs/<serial>.pem (new file — cleanup target on failure).
// 11. WriteCounter (serial + 1) via WriteFileAtomic.
//     If fails: RemoveIfExists(certPath), return error.
// 12. Append IndexEntry to index, SaveIndex via WriteFileAtomic.
//     If fails: RemoveIfExists(certPath), WriteCounter(oldSerial) to restore, return error.
...
// MUTATE PHASE (atomic write per ADR-003):
// 5. Set entry status="revoked", revoked_at=time.Now().UTC().Format(RFC3339),
//    revocation_reason=reason.
// 6. SaveIndex via WriteFileAtomic (single atomic write — no cleanup needed).
...
// MUTATE PHASE (atomic writes + ordered cleanup per ADR-003):
// 7. SaveCRLPEM to ca.crl via WriteFileAtomic (CON-DI-013).
// 8. WriteCounter for crlnumber + 1 via WriteFileAtomic (CON-INV-007, CON-DI-009).
//    If fails: restore previous CRL or RemoveIfExists(crlPath), return error.
// 9. Return CRLResult.
```
...
Error handling follows the **two-layer atomicity strategy** (see ADR-003):

- **Layer 1 (validate-before-mutate):** All validation occurs before any state is modified. If validation fails, the system returns an error and exits without touching persistent state.
- **Layer 2 (atomic writes + ordered cleanup):** During the mutate phase, file writes use `WriteFileAtomic` (temp file + atomic rename) and multi-file mutations use ordered cleanup on failure. If an I/O error occurs mid-mutation, earlier writes are rolled back via `RemoveIfExists` and counter restoration.

Together, both layers satisfy CON-DI-004 ("failed operations shall not modify state") for logical failures and I/O failures alike.
...
| [ADR-003](ADRs/ADR-003-validate-before-mutate-atomicity.md) | Validate-Before-Mutate with Atomic Writes for Operation Atomicity | Two-layer strategy: (1) validate-before-mutate for logical errors, (2) WriteFileAtomic + ordered cleanup for I/O failures; reject validate-only approach |
...
| CON-DI-004 | CA Operations + CRL + Storage | ca.go, crl.go, store.go | Two-layer atomicity (ADR-003): Layer 1 — all validation checks complete before any file writes; Layer 2 — WriteFileAtomic (temp+rename) for individual writes, ordered cleanup (RemoveIfExists + counter restoration) for multi-file mutations |
...
| CON-DI-007 | CA Operations + Storage | ca.go, store.go | SignCSR writes cert file, serial, and index in order with cleanup on failure (ADR-003 Layer 2); if index write fails, cert file is removed and serial restored — no orphaned files or entries |
